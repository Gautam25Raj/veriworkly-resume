...
<Callout type="warn">
  The manual `/sync` endpoint is strictly protected. Unauthorized calls will be
...
>
  ...
  Retrieve a paginated list of repository items. Supports advanced query
  filtering by development `status` (todo, in-progress, done) and `kind` (issue,
  pull-request).
</Card>
