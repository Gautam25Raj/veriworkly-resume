...
| `REDIS_URL`    | Yes\*    | Unified Redis connection string.         | `redis://localhost:6379`              |
...
| Variable                   | Description                                   | Default         |
| :------------------------- | :-------------------------------------------- | :-------------- |
| `EXPORT_QUEUE_NAME`        | BullMQ queue identifier in Redis.             | `resume-export` |
| `EXPORT_MAX_CONCURRENCY`   | Maximum concurrent rendering instances.       | `2`             |
| `EXPORT_ARTIFACT_PROVIDER` | Storage backend selection (`local` or `oci`). | `local`         |
| `EXPORT_TIMEOUT_MS`        | Maximum execution duration for a single job.  | `45000`         |
| `EXPORT_RESULT_TTL_MS`     | Metadata retention duration in Redis.         | `600000`        |
